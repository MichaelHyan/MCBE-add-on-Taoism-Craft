# [add-on]道教仙术扩展包

![](Taoism_R/title.png)

## 一个融合传统道教文化与奇幻玩法的add-on
## 道教之道，法于自然，和于阴阳，通于鬼神。
## 此扩展包秉承道家“天人合一”之理念，
## 将千年道教文化精髓融入方块世界，
## 让玩家在探索中体悟“道生一，一生二，二生三，三生万物”之妙，
## 感受符箓、法器、修行之趣，于游戏中窥见道教文化的深邃与博大。

[演示视频](https://www.bilibili.com/video/BV118iiBNEvh/)

### 使用方式

下载TaoismV5.mcaddon并使用我的世界基岩版打开

支持版本1.21.130以上

低版本部分功能可能失效(如长矛不存在，北剑索敌失效等)

### 温馨提示
- 模组制作过程中，部分内容的参考文献源自明清时代的部分典籍或著作，
- 由于时代较近，部分玩家会将其归为“志怪小说”或“神话故事”，
- 所以请各位玩家理性看待模组内容，
- 我们会尽可能的为大家呈现最原汁原味的道教仙术的体验。

### 整合内容：战斗优化

整合自模组：战斗优化。所有原版武器均获得增强。
- 剑类
  - 所有剑拥有格挡和横扫能力
  - 横扫仿自java版横扫效果，伤害为面板的一半向上取整，有短暂cd
  - 未装备盾牌时下蹲使用格挡，获得抗性3以及饥饿效果
- 重剑类
  - 加入新武器类型【重剑】
  - 持重剑下蹲时可进行蓄力，共可蓄力三段，跳劈时将释放蓄力并造成范围伤害
  - 蓄力一段跳劈造成50%面板额外伤害，二段100%，三段150%
  - 合成方式为原版剑斜向摆放
  - 面板，耐久，修复值和附魔与原版相同
- 长矛类
  - 持有时冲刺速度增快
- 远程类
  - 大幅减少投射物弹道下坠
  - 少许增加雪球伤害
  - 持有弓时下蹲缩小视野,左键攻击回收箭，下落一定速度后点按攻击进入子弹时间
  - 持有弩时下蹲大幅缩小视野并附加抗性提升
- 三叉戟
  - 浸入水中或雨中时获得暂时增伤效果
- 重锤
  - 下蹲时攻击可起跳
  - 从极高处掉落时不再受到掉落伤害
- 盾牌
  - 增加攻击方式【盾冲】
  - 增加反击招式
- 指南针
  -手持指南针时额外显示方位
- 其他
  - 驯服的猫可能会带来更有价值的礼物
  - 自体技能【无伤】，在血条全满时稍有攻击力提升
  - 自体技能【火场怪力】，在血条较低时稍有攻击力提升和防御力提升，血条极低时攻击力和防御力大幅提升
  - 加入自体技能【潜伏】，潜行一段时间后进入潜伏状态，可在此状态发动破隐攻击

### 自然生成建筑类
#### 道馆（正一派）
- 生成机制：世界自然生成，为玩法起始核心场景
- 核心功能：
  1. 明示投币
  2. 内部存放丰富资源（基础材料、引导道具等）
  3. 玩家可获取引导书，开启道教主题玩法剧情

![](crafting%20table/temple1.png)

“道之所存，馆宇所栖，修行由此启，大道自此寻。”

#### 竖穴石室墓
- 生成机制：世界自然生成，为玩法扩展场景，属于中等官员的大型墓穴
- 核心功能：仿照真实的墓穴场景搭建，充满刺激的探险与丰厚的奖励并存。

“潜寐黄泉下，千载永不寤。”

#### 甲字型墓
- 生成机制：世界自然生成，为玩法扩展场景，属于普通官员的中型墓穴
- 核心功能：仿照真实的墓穴场景搭建，充满刺激的探险与丰厚的奖励并存。

“幽室一已闭，千年不复朝。”

### 等级
所有物品拥有经验等级限制，等级未达到时不影响合成和面板数值，但无法激活特殊效果

### NPC

#### 游商

白天出现在地表，使用弓自卫，可以使用朱砂交换秘籍和家具。

- 村子周边经常看到一位赶路的老人，他衣冠不整，也不像是现代的人，自称游商。
- 偶尔有人提及，有长寿者惊恐，问其状貌，疾呼这并非人。
- 不是所有鬼魅成精是为了害人，偶然也有体验人间烟火的存在。

### 敌对生物

#### 旱魃

阴气最重的晚上（满月之夜）或新月之夜会随机生成。拥有极高的血量和伤害，可强化其他僵尸，但受雷系法术克制，有可能会掉落尸丹。
旱魃生成之后会持续强制改天气为晴天，同时若遭强行降雨或被水浸泡则会持续扣血。

- 山脚下佝偻的老道总叮嘱游人，满月与新月两夜，务必关好门窗，别往后山看。
- 传言百年前此地大旱三月，有个替村人求雨的女子被诬为灾星，沉了后山枯井。
- 自那以后每逢月相两极之夜，井里便走出个焦衣女子，她走过的地方草枯土裂，唯有惊雷落雨时，才敢缩回井底。

#### 跳僵（尸）

夜晚会随机生成，拥有极高的血量，但移动速度较慢，击败后有可能会掉落尸丹碎片。

- 相传在后坡有座无人祭扫的竖穴石室墓，老一辈都叫它 “跳尸坟”。
- 说是前清时迁坟漏了一具棺，尸身受了山阴之气，夜里会撑着棺盖蹦出来，一步一顿顺着山道走，走得极慢却从不迷路。
- 村里人说撞见了别跑，贴着石敢当站着，它辨不清活人气，蹦到天亮自己就“回”坟里去了。

#### 飞僵（尸）

夜晚会随机生成，血量比跳僵略低，但移动速度飞快，且拥有远程攻击与自我恢复/吸血的能力，击败后有可能会掉落尸丹碎片。

- 村西陡崖上有处荒弃的岩葬洞，老一辈都叫它 “飞尸洞”。
- 传说是古时一个劫道的飞贼被射杀在崖下，尸身没人收，浸了百年山阴瘴气，夜里能展着硬邦邦的尸衣滑翔，掠着树梢来去如风。
- 人还没瞧见影子，颈侧已经添了血口；撞见了别追，贴着土坡伏下身，它眼拙看不清低处活气，擦着头皮就飞远了。

#### 伏尸

地下少量生成，平时无法发现，玩家靠近后会突然出现并攻击，顷刻消失。

- 村南荒坡乱葬岗，老一辈都叫它 “伏尸窝”。
- 说是早年战乱抛下的无名尸都埋在浅土里，尸身浸了地阴之气，便伏在土下蛰伏不动；有人打跟前走过，就猛地破土探出头来吓人，前后不过眨眼功夫，便又缩回土里消无声息。
- 村里人说走夜路撞见了别跑，它只惊生人不伤人，等它缩回去绕开新翻的土走便没事，千万别伸手去扒土。

#### 玩家的尸体

玩家死亡后留下尸体。尸体5分钟左右尸变，开始活动。此时仍可以镇压。如置之不理，10分钟后将转化为跳僵。使用普通符镇压后，可以在阳光下再次使用普通符火化。

- 前山落石坡的乱石坳，过往的游侠都唤作 “守尸坪”。
- 修道人在此殒命，尸首若不及早收殓，虽也会因地阴之气化尸，却不似野尸四处游荡，只守着自己散落的行囊法器，半步不肯挪开；生人靠近便发狂扑击，唯独携着它生前信物时，动作会迟滞几分。
- 寻回自身尸首时，将朱砂符端正贴于眉心，便可镇住尸身戾气，它会抱起遗物跟在身后，一路安分；若是符纸贴歪，它便会扔下行囊，顺着山坡直往下冲，再难唤回。

#### 鬼

鬼会且仅会在”闹鬼事件“中出现，“闹鬼事件”在玩家睡觉时会有七分之一的概率发生。鬼会持续干扰玩家的睡眠，且在一定时间后会因阳气侵蚀自动消散。

- 在村子的西厢房第三间，老人都知道 “梦扰” 的说法。
- 相传早年有个可怜的村民在这里被折磨致死，执念着剩下的生命，每到有人夜宿时，便会趁着人睡沉了凑到床边，想借点阳气续魂。
- 人半梦半醒间总觉得耳边有呼吸、被角被轻拽，可一旦彻底睁眼醒透，那东西受不住阳气，慢慢的就散得无影无踪。

#### 神秘道士

在末地击败末影龙后与神秘遗物互动后出现，如果你能查明他的过往，可以获得一件特殊法宝。

### 特殊机制
#### 赶尸

玩家死亡后留下尸体。尸体5分钟左右尸变，在尸变前可以用普通的符箓镇压尸体体内的邪气，并整理好尸体的衣着与行囊，尸体会跟随玩家回到某个地方。

- 村北旧驿道旁的荒草坡，在村志中被记载为 “赶尸径”。
- 但凡有人在此殒命，尸首若不及时收殓，时日一久便会沾了地阴之气自行起身，成了沿路游荡的行尸；若能寻回尸首，以朱砂符贴于额头，便能引着它缓缓随行，送归故土安葬。
- 只是贴符时务必端正，符纸一歪便镇不住尸气，它便失了方向，只知顺着旧驿道直挺挺地前行，再也唤不回头。

### 经书类
#### 经书（合成）
- 核心功能：
  1. 使用后永久增益：最大血量提升100%
  2. 使用后获得1个「内丹」（核心物品）
  3. 可用于开启古书支线
- 副作用：一次性物品，使用后消耗，若在亏凹月和盈凹月以外的日子修炼，会修炼失败，遭受雷劫
- 合成方式：7个朱砂粉 + 1个普通毛笔 + 1本书
- 特殊经书：使用奖励箱中的经书不会失败

![](crafting%20table/scripture_t.png)

“真经载道，开悟明心，得之则内丹生，体健身轻。”

#### 噬魂术（经书）
- 核心功能：合成材料（用于制作邪道相关物品）
- 副作用：象征「误入歧途」修炼路径，无直接使用效果
- 获取方式：击败掠夺者后掉落

![](Taoism_R/textures/items/scripture_s.png)

“噬魂之术，逆道而行，虽得暂力，终堕幽冥。”

#### 轻功（经书）
- 核心功能：从经书中分离而成，使用后获得速度提升和跳跃提升
- 副作用：一次性物品，使用后消耗
- 合成方式：1个经书 + 1个羽毛

![](crafting%20table/scripture_f.png)

“御风而行，不滞于形，此乃得道者动静之妙。”

#### 古书
- 核心功能：很有年代的书，目前的文字看不懂，但日后文字会渐渐浮现
- 获取方式：？

![](Taoism_R/textures/items/acbook.png)

“所谓修仙，不过是复归大道本源，却是无数人穷尽一生所求。到头来，那还算得上是修仙吗？”

- 天道崩毁前，也曾是一番盛世。那时候人们还相信，修仙可以解决任何问题。
- 这村子原本有个大侠，像是信道，倒是没见他修炼过，似乎和那观中道长交往甚好。
- 人被杀，聚集的怨气变了厉鬼，但近年盛行杀人放火，却不见恶鬼横行，倒是地主腰间的木头玩意似乎是怨气越来越重。

#### 宝经卷轴
- 核心功能：使用后获得大量经验值，助力修行境界提升
- 副作用：一次性物品，使用后消耗
- 合成方式：1个元气 + 1个钻石 + 1个绿宝石 + 4个金锭

![](crafting%20table/txpa.png)

“宝轴藏玄理，开卷蕴真智，一念悟得经中义，海量经验助修行。”

#### 秘籍-天罡雷法
- 核心功能：可学习/装备
- 使用方式：
  - 内丹【法】模式下，绑定该秘籍，使用内丹交互
  - 交互后获得雷法，任意攻击可释放剑气，剑气造成少量伤害和减速效果
  - 雷法能量随等级提高而增加，攻击时消耗能量，直到耗尽
- 获取方式：游商处购买

![](Taoism_R/textures/items/sebookb.png)

”天罡指处有雷霆，便向其中役六丁。若解个中些子诀，信知造化掌中生。“

#### 秘籍-制炼七魄
- 核心功能：可学习/装备
- 使用方式：
  - 内丹【法】模式下，绑定该秘籍，使用内丹交互
  - 交互后发射7个粒子，分别可造成一次伤害
  - 粒子伤害随等级提高而增加
- 获取方式：游商处购买

![](Taoism_R/textures/items/sebookc.png)

“制炼幽魄化七锋，内丹运处迸星踪。功高每道增威势，散击群邪一瞬空。”

#### 秘籍-存思内景
- 核心功能：可学习/装备
- 使用方式：
  - 内丹【法】模式下，绑定该秘籍，使用内丹交互
  - 静止时获得额外生命恢复
  - 交互后短暂获取移速加成
  - 加成类型随等级提高而增加
- 获取方式：游商处购买

![](Taoism_R/textures/items/sebookd.png)

“存思内景百骸融，静养元气血自充。运法顿生追风步，功深捷势愈无穷。”

#### 秘籍-练体
- 核心功能：可学习/装备
- 使用方式：
  - 内丹【法】模式下，绑定该秘籍，使用内丹交互
  - 交互后展开法阵，法阵生效期间对法阵内部地对生物产生伤害，给友方单位加成效果
  - 加成类型随等级提高而增加
- 获取方式：游商处购买

![](Taoism_R/textures/items/sebooke.png)

“炼体凝罡布法坛，阵中邪秽自摧残。还将元气资同道，功愈深时效愈宽。”

### 基础材料类
#### 内丹
- 核心功能：运气，调出元气
- 特殊说明：元气乃生命能量，提元会损耗寿元
- 获取方式：使用「经书」后解锁

![](Taoism_R/textures/items/pearl.png)

“内丹者，性命之根，天地灵气所凝，能化万象。”

#### 元气
- 核心功能：
  1. 合成材料（用于制作符箓、武器、特殊道具）
  2. 可通过「聚气」将元气攒起来为「息」，3个元气为1「息」，最多攒3「息」
  3. 拥有「息」时可立刻取消部分法器的冷却时间，每次使用1「息」
- 特殊说明：元气无形无相，动则生阳，耗则需补，乃修行之根基
- 获取方式：使用「内丹」运气调出

![](Taoism_R/textures/items/spe.png)

“元气者，性命之本，先天真炁所聚，可通万法。”

#### （元）气弹/炁弹
- 核心功能：
  1. 使用大还丹后解除内丹限制，由内丹调出
  2. 共有金、木、水、火、土五种属性
  3. 命中后造成少量伤害和元素标记，使用相生元素再次命中目标后会清除上一个标记并造成大量结算伤害
  4. 每个气弹都有相应的主动技能，到达进阶等级后下蹲+攻击释放，冷却极短
- 特殊说明：无基础等级，不同属性的气弹进阶等级限制也不同，需达到等级才可使用主动技能
- 主动技能：
  1. 金元素：25级进阶
  - 被动效果：可使用特殊的自反应，直接制造特殊元素标记并自动结算特殊标记伤害，无需主动发动
  - 主动效果：强制清除所有元素标记并直接造成结算伤害
  2. 木元素：10级进阶
  - 1分钟内加速植物生长。注意：可能造成卡顿
  3. 水元素：15级进阶
  - 短暂治疗效果
  4. 火元素：10级进阶
  - 召唤灯笼，灯笼可存在3分钟，可使用火气弹攻击敌怪，该攻击可以附加火元素标记
  5. 土元素：15级进阶
  - 召唤龙卷风，将自己和其他生物吹上天，龙卷风会优先移动至存在的土气弹处
- 获取方式：使用「内丹」运气调出

![](Taoism_R/textures/items/spba.png)
![](Taoism_R/textures/items/spbb.png)
![](Taoism_R/textures/items/spbc.png)
![](Taoism_R/textures/items/spbd.png)
![](Taoism_R/textures/items/spbe.png)

“内丹吐炁结星弹，五德分形性各安。逢生触发玄机变，一击摧邪煞力寒。”

#### 炁
- 核心功能：
  - 在世界中随机生成，用内丹吸收后会掉落物品「炁」，「炁」为合成材料（用于制作部分法器或武器）
  - 等级足够时可将其凝聚炼化为其他物品
- 特殊说明：炁为天地之本，万物之根

![](Taoism_R/textures/items/qi.png)

“炁为天地之本，万物之根，聚则成形，散则归无，通于道而贯于形。”

#### 普通毛笔
- 核心功能：合成材料（用于制作「画符毛笔」）
- 合成方式：2个木棍 + 1个白色羊毛 可合成4个

![](crafting%20table/brush_n.png)

“笔为心之器，墨为道之痕，可书符咒，可绘阴阳。”

#### 朱砂晶体
- 核心功能：合成材料（用于制作「朱砂粉」）
- 特殊说明：道教道具制作的基础材料，「初见端倪」
- 获取方式：击败亡灵生物后掉落

![](Taoism_R/textures/items/cinnabar_crystal.png)

“朱砂禀离火之气，色赤通神，辟邪制煞，为道之灵材。”

#### 朱砂粉
- 核心功能：合成材料（用于制作画符毛笔、桃木剑等）
- 合成方式：1个元气 + 1个朱砂晶体（可合成3个朱砂粉）

![](crafting%20table/cinnabar.png)

“丹砂为粉，符力自生，一点通灵，镇伏邪祟。”

#### 画符毛笔
- 核心功能：合成材料，用于制作各类符箓，制作后会返还普通毛笔
- 合成方式：1个毛笔 + 1个朱砂粉

![](crafting%20table/brush_p.png)

“画符之笔，蘸朱砂之灵，运天地之气，落笔则鬼神应。”

#### 魂魄
- 核心功能：
  1. 合成材料（用于制作灵剑、万魂幡），给万魂幡充能
  2. 可被净瓶净化为元气
- 副作用：主手持有时，玩家持续获得凋零效果
- 获取方式：击败任意生物后掉落

![](Taoism_R/textures/items/spirit.png)

“魂为阳之精，魄为阴之质，聚散有常，顺乎天道。”

#### 鱼鳔
- 核心功能：合成材料（用于制作鱼胶）
- 获取方式：1个生鱼肉可合成3个鱼鳔

![](Taoism_R/textures/items/fish_belly.png)

“鱼鳔聚水之精，炼化为胶，可固符形，为制符之辅。”

#### 鱼胶
- 核心功能：合成材料（用于制作高阶符箓）
- 合成方式：将鱼鳔放置于篝火上烧制生成

![](Taoism_R/textures/items/fish_gelatin.png)

“鱼胶凝鳔之华，黏性强韧，聚符气而固其效。”

#### 尸丹碎片
- 核心功能：合成材料（用于合成尸丹）
- 获取方式：击败跳僵有概率获得

![](Taoism_R/textures/items/pearlz_p.png)

“枯骸凝煞结残丹，碎屑犹含蚀骨寒。莫嫌微物无多用，邪器炼成赖此端。”

#### 尸丹
- 核心功能：合成材料，用于合成部分武器，后期可能会设定为邪修常用道具（更新预告）
- 获取方式：击败旱魃会获得一个尸丹

![](crafting%20table/pearlz_o.png)

“枯尸历劫结丹砂，旱气阴精萃一家。留与旁门铸邪刃，一挥赤地起尘沙。”

#### 炼化尸丹
- 核心功能：合成材料，用于合成净炼尸丹
- 合成方式：8个元气 + 1个尸丹

![](crafting%20table/pearlz_s.png)

“元炁烹丹涤秽痕，阴尸残煞渐消泯。一番炼化除浊质，留取清基作净根。”

#### 净炼尸丹
- 核心功能：合成材料，用于合成部分武器，后期可能会设定为正派常用道具（更新预告）
- 合成方式：1个普通符箓 + 2个朱砂粉 + 1个炼化尸丹 + 1个元气

![](crafting%20table/pearlz.png)

“以符涤浊净阴丹，朱砂凝阳化旧寒。洗尽尸中千缕煞，堪为正道铸锋翰。”

#### 大还丹
- 取自周星驰老师的电影《武状元苏乞儿》里的大还丹，原型是《太清石壁记》记载的九鼎大还丹
- 核心功能：当在5级以上时使用大还丹后，可切换内丹来引出不同属性的元气（气弹），同时解锁内丹法术模式
- 合成方式：4个钻石 + 1个黏土 + 1个火药 + 1个金粒 + 1个朱砂粉 + 1个元气

![](crafting%20table/pilla.png)

“呢粒大还丹，你食咗之后会功力大增。”

#### 小还丹
- 取自周星驰老师的电影《武状元苏乞儿》里的大还丹，原型是《太清石壁记》记载的九鼎大还丹
- 核心功能：在“闹鬼事件”发生时，可以通过食用小还丹来暂时获得充沛的阳气，以达到避免鬼魂干扰睡眠的效果
- 合成方式：1个黏土 + 1个火药 + 1个金粒 + 1个朱砂粉 + 1个元气

![](crafting%20table/pillb.png)

“呢粒大还丹，你食咗之……诶不对？”

#### 阴魄砂
- 核心功能：合成材料
- 合成方式：伏尸掉落/掠夺者前哨塔宝箱获得

![](Taoism_R/textures/items/soul_powder.png)

"伏尸历劫结阴砂，魄气寒精萃一家。留与旁门铸邪刃，寒芒锋底带幽花。"

#### 残符【三清】
- 核心功能：合成材料（用于制作金光符、九木镇风符等三清系列符箓），自带法力
- 合成方式：3个朱砂粉 + 3张纸

![](Taoism_R/textures/items/ta.png)

“残符【三清】承三清道力，虽为碎片，含天地本源，乃高阶符箓之基。”

#### 残符【敕令】
- 核心功能：合成材料（用于制作补金符、补木符等敕令系列符箓），自带法力
- 合成方式：1个元气 + 1个桃木剑 + 1张纸

![](Taoism_R/textures/items/tb.png)

“残符【敕令】携天庭之威，符文藏势，可引天地元素为用。”

#### 残符【雷令】
- 核心功能用于制作雷击用于制作雷击符、金雷镇煞符等雷令系列），自带法力
- 合成方式：1个元气 + 1个避雷针 + 1张纸

![](Taoism_R/textures/items/tc.png)

“残符【雷令】聚天雷之威，符文如雷纹，可召雷霆破邪。”

#### 符脚【金】
- 核心功能：合成材料（用于制作金雷镇煞符、金光符），自带法力
- 合成方式：1个画符毛笔 + 1个铁剑 + 1张纸

![](Taoism_R/textures/items/ua.png)

“符脚【金】禀金之刚，落笔带锋，增符箓破邪增益之力。”

#### 符脚【木】
- 核心功能：合成材料（用于制作青木雷护符、九木镇风符、补木符），自带法力
- 合成方式：1个画符毛笔 + 1个骨头 + 1张纸

![](Taoism_R/textures/items/ub.png)

“符脚【木】含木之生，落笔蕴灵，增符箓续航控场之力。”

#### 符脚【水】
- 核心功能：合成材料（用于制作雷水解厄符、净水符、补水符），自带法力
- 合成方式：1个画符毛笔 + 1个墨囊 + 1张纸

![](Taoism_R/textures/items/uc.png)

“符脚【水】携水之智，落笔随流，增符箓环境适应之力。”

#### 符脚【火】
- 核心功能：合成材料（用于制作炎雷破邪符、离火符、补火符），自带法力
- 合成方式：1个画符毛笔 + 1个火药 + 1张纸

![](Taoism_R/textures/items/ud.png)

“符脚【火】聚火之烈，落笔燃威，增符箓攻击灼烧之力。”

#### 符脚【土】
- 核心功能：合成材料（用于制作厚土雷安符、龙神土符、补土符），自带法力
- 合成方式：1个画符毛笔 + 1个泥土 + 1张纸

![](Taoism_R/textures/items/ue.png)

“符脚【土】承土之厚，落笔镇稳，增符箓防御创造之力。”

#### 符脚【风】
- 核心功能：合成材料（用于制作雷风破煞符、缩地符、补风符），自带法力
- 合成方式：1个画符毛笔 + 1个风弹 + 1张纸

![](Taoism_R/textures/items/uf.png)

“符脚【风】携风之速，落笔随变，增符箓迅捷瞬移之力。”

#### 符脚【三台星】
- 核心功能：合成材料（用于制作雷令镇垣三台符、三清镇元三台符、敕令安垣三台符），自带法力
- 合成方式：1个符脚【水】 + 1个符脚【火】 + 1个符脚【土】 + 2个鱼胶 + 1个画符毛笔

![](Taoism_R/textures/items/ug.png)

“三台星者，天之三阶，符脚承其力，合水火土之精，聚三才之气。”

### 符箓类
#### 普通的符箓
- 核心功能：
  1. 可投掷，使用时对生物有镇压（禁止其移动）的效果
- 副作用：一次性物品，使用后消耗
- 合成方式：1个画符毛笔 + 1张纸

![](Taoism_R/textures/items/talisman.png)

“符箓者，天地之令，阴阳之契，可驱邪鬼，可安生灵。”

#### 雷击符
- 核心功能：
  1. 主手持有时：移速提升、抗性提升
  2. 使用时(右键/交互键)：会借用太上老君的法力，在周身降下天雷，天雷会自动寻找目标劈下
- 副作用：一次性物品，使用后消耗
- 合成方式：1个画符毛笔 + 1个残符【雷令】 + 1张纸 + 1个元气 + 1个鱼胶

![](Taoism_R/textures/items/talisman_t.png)

“雷为天怒，符引其威，雷击所及，邪祟消亡。”

#### 缩地符
- 核心功能：记录当前位置风水，可主动触发瞬移（包含笏的瞬移至亲友功能）
- 合成方式：1个画符毛笔 + 1个残符【三清】 + 1个符脚【风】 + 1个鱼胶
- 使用方式：包含笏的效果，左键/攻击键记录位置，并在原地生成一个微型土地庙，随后随时可使用shift/潜行键回到土地庙，的位置，可同时存在多个位置，所有玩家共享，按住space/跳跃的同时按下shift/潜行删除所有记录地点

![](Taoism_R/textures/items/talisman_e.png)

“缩地成寸，道法自然，一念千里，此乃通玄之妙。”

#### 金光符
- 核心功能(右键/交互键)：使用效果等同于金苹果（恢复生命值+基础增益）
- 副作用：一次性物品，使用后消耗
- 合成方式：1个画符毛笔 + 1个残符【三清】 + 1个符脚【金】 + 1个鱼胶

![](Taoism_R/textures/items/talisman_g.png)

“金光护体，百邪不侵，得此符者，身有灵光，福寿康宁。”

#### 金雷镇煞符
- 核心功能(右键/交互键)：使用时使用者获得“三五雌雄斩邪剑”buff（将秒杀效果调整为大幅增加攻击力）
- 副作用：1. 一次性物品，使用后消耗；2. buff持续时间仅10秒
- 合成方式：1个画符毛笔 + 1个残符【雷令】 + 1个符脚【金】 + 1个鱼胶

![](Taoism_R/textures/items/tca.png)

“金雷镇煞，锋芒毕露，借剑符之力，短时增攻，破邪无往不利。”

#### 青木雷护符
- 核心功能(右键/交互键)：使用时使用者最大血量提升400%，同时获得抗性提升与持续回血效果
- 副作用：1. 一次性物品，使用后消耗；2. 所有效果仅持续20秒
- 合成方式：1个画符毛笔 + 1个残符【雷令】 + 1个符脚【木】 + 1个鱼胶

![](Taoism_R/textures/items/tcb.png)

“青木蕴生机，雷力护其身，此符在手，血厚防高，续航无忧。”

#### 雷水解厄符
- 核心功能(右键/交互键)：
  1. 引一道天雷劈向自身（无伤害），将当前天气改变为雷暴天
  2. 为使用者增加持续回血与漂浮buff
  3. 效果结束后自动获得缓降buff
- 副作用：1. 一次性物品，使用后消耗；2. 回血与漂浮buff仅持续10秒，缓降buff持续30秒
- 合成方式：1个画符毛笔 + 1个残符【雷令】 + 1个符脚【水】 + 1个鱼胶

![](Taoism_R/textures/items/tcc.png)

“雷水解厄，逆天气而变，借天雷之力获漂浮续航，危难时可破局。”

#### 炎雷破邪符
- 核心功能(右键/交互键)：
  1. 以使用者为中心，在10×10×10范围内降下天雷（对敌人造成伤害）
  2. 在周身15×15×15范围内生成火焰（灼烧敌人）
  3. 使用者自身获得火抗buff，可主动熄灭周身火焰
- 副作用：一次性物品，使用后消耗
- 合成方式：1个画符毛笔 + 1个残符【雷令】 + 1个符脚【火】 + 1个鱼胶

![](Taoism_R/textures/items/tcd.png)

“炎雷交织，水火共威，天雷焚火齐降，邪祟无所遁形。”

#### 厚土雷安符
- 核心功能(右键/交互键)：
  1. 以使用者为中心，生成3×3×3的土房子（可永久存在，作为临时庇护所）
  2. 在周身10×10×10范围内随机降下雷决（密度适中，对敌人造成伤害）
- 副作用：1. 一次性物品，使用后消耗；2. 降雷效果仅持续10秒
- 合成方式：1个画符毛笔 + 1个残符【雷令】 + 1个符脚【土】 + 1个鱼胶

![](Taoism_R/textures/items/tce.png)

“厚土筑屋安，雷决护周旁，此符可攻可守，危急时能安身。”

#### 雷风破煞符
- 核心功能(右键/交互键)：
  1. 使用者获得最大等级的速度提升buff
  2. 对周身5×5×5范围内的所有生物施加减速效果
- 副作用：1. 一次性物品，使用后消耗；2. 所有效果仅持续15秒
- 合成方式：1个画符毛笔 + 1个残符【雷令】 + 1个符脚【风】 + 1个鱼胶

![](Taoism_R/textures/items/tcf.png)

“雷风助速，破煞制敌，己快敌慢，战场之上可掌主动权。”

#### 九木镇风符
- 核心功能(右键/交互键)：以使用者为中心，调节20×20×20范围内的风水气场，使范围内所有生物向上漂浮20秒后自然下落（无伤害，仅控场）
- 副作用：一次性物品，使用后消耗
- 合成方式：1个画符毛笔 + 1个残符【三清】 + 1个符脚【木】 + 1个鱼胶

![](Taoism_R/textures/items/tab.png)

“九木镇风，控场自如，借风水之力扰敌走位，为己创造先机。”

#### 净水符
- 核心功能(右键/交互键)：使用后使用者获得夜视、水下呼吸、水下速掘等所有水下相关增益buff
- 副作用：1. 一次性物品，使用后消耗；2. 所有buff持续2分钟
- 合成方式：1个画符毛笔 + 1个残符【三清】 + 1个符脚【水】 + 1个鱼胶

![](Taoism_R/textures/items/tac.png)

“净水符出，水下无阻，探索如履平地。”

#### 离火符
- 核心功能(右键/交互键)：以使用者为中心，生成一个永久光源，形成“紫气东来”效果（照亮区域，驱避邪祟）
- 副作用：一次性物品，使用后消耗
- 合成方式：1个画符毛笔 + 1个残符【三清】 + 1个符脚【火】 + 1个鱼胶

![](Taoism_R/textures/items/tad.png)

“离火照明，紫气驱邪，黑暗邪祟皆不敢近。”

#### 龙神土符
- 核心功能(右键/交互键)：
  1. 使用者获得庇佑效果，改变周身50×50×50范围内的气场
  2. 对范围内所有生物施加持续震慑效果（使其行动迟缓、攻击力降低）
- 副作用：1. 一次性物品，使用后消耗；2. 效果仅持续15秒；3. 使用者自身因承受气场之力，会持续扣血
- 合成方式：1个画符毛笔 + 1个残符【三清】 + 1个符脚【土】 + 1个鱼胶

![](Taoism_R/textures/items/tae.png)

“龙神护土，气场镇敌，虽自损精血，却能大范围制敌，险中求胜。”

#### 补金符
- 核心功能(右键/交互键)：在使用者四面八方召唤普通铁傀儡（无额外buff，仅基础战斗能力）
- 副作用：一次性物品，使用后消耗
- 合成方式：1个画符毛笔 + 1个残符【敕令】 + 1个符脚【金】 + 1个鱼胶

![](Taoism_R/textures/items/tba.png)

“补金召傀儡，助战守身旁，可挡敌锋，护己安全。”

#### 补木符
- 核心功能(右键/交互键)：以使用者为中心，在10×10×10范围内自动生成橡木树苗
- 副作用：一次性物品，使用后消耗
- 合成方式：1个画符毛笔 + 1个残符【敕令】 + 1个符脚【木】 + 1个鱼胶

![](Taoism_R/textures/items/tbb.png)

“补木生橡木，建材随手得，探索途中缺木料，此符一用解燃眉。”

#### 补水符
- 核心功能(右键/交互键)：
  1. 以使用者为中心，生成5×5×5范围的水（可用于灌溉、灭火或创造水下环境）
  2. 下蹲时使用收回生成的水
- 副作用：一次性物品，使用后消耗
- 合成方式：1个画符毛笔 + 1个残符【敕令】 + 1个符脚【水】 + 1个鱼胶

![](Taoism_R/textures/items/tbc.png)

“补水聚水源，收放皆随心，缺水环境中，此符可造水、可控水。”

#### 补火符
- 核心功能(右键/交互键)：为使用者及周围队友同时提供持续回血效果，并增加一定移速
- 副作用：1. 一次性物品，使用后消耗；2. 所有效果仅持续30秒
- 合成方式：1个画符毛笔 + 1个残符【敕令】 + 1个符脚【火】 + 1个鱼胶

![](Taoism_R/textures/items/tbd.png)

“补火暖身心，回血加移速，团队作战时，此符可增续航与机动性。”

#### 补土符
- 核心功能(右键/交互键)：使用后，使用者跳跃时，脚下会自动生成土块作为垫脚（可用于攀爬、过河等），需手动关闭功能
- 合成方式：1个画符毛笔 + 1个残符【敕令】 + 1个符脚【土】 + 1个鱼胶

![](Taoism_R/textures/items/tbe.png)

“补土垫脚，攀爬无阻，跳跃生土块，险峻地形亦可轻松跨越。”

#### 补风符
- 核心功能(右键/交互键)：使用者长按鼠标右键蓄力，即可在准星处不断聚集风弹。
- 副作用：非常危险的符箓，慎用
- 合成方式：1个画符毛笔 + 1个残符【敕令】 + 1个符脚【风】 + 1个鱼胶

![](Taoism_R/textures/items/tbf.png)

“运气成风，蓄力御风，攻防一体，孤身作战亦无惧。”

#### 开光符【闭灵】
- 新月时使用可获得开光符【启灵】

![](crafting%20table/echa.png)

"新月凝灵闭中藏，待得弦时启锋芒。"

#### 开光符【启灵】
- 下弦月/上弦月置于副手使用时可开光剑类武器

![](Taoism_R/textures/items/echb.png)

"弦月照剑锋，启灵开道芒，一符通灵性，凡铁化天罡。"

#### 雷令镇垣三台符
- 核心功能(置于副手)：为武器类物品附魔火焰附加
- 合成方式：1个画符毛笔 + 1个残符【雷令】 + 1个符脚【三台星】 + 1个鱼胶

![](Taoism_R/textures/items/tcg.png)

“雷令召三台，镇垣筑雷墙，屏障护友军，反伤惩来敌。”

#### 三清镇元三台符
- 核心功能(置于副手)：为工具类物品附魔时运
- 合成方式：1个画符毛笔 + 1个残符【三清】 + 1个符脚【三台星】 + 1个鱼胶

![](Taoism_R/textures/items/tag.png)

“三清护三台，镇元固生机，灵光驱邪祟，群体增益护周全。”

#### 敕令安垣三台符
- 核心功能(置于副手)：为武器类物品附魔抢夺
- 合成方式：1个画符毛笔 + 1个残符【敕令】 + 1个符脚【三台星】 + 1个鱼胶

![](Taoism_R/textures/items/tbg.png)

“敕令统三台，安垣固方块，气场助队友，攻防皆宜稳阵脚。”

#### 符箓卷轴
- 核心功能：使用后获得游戏内所有正面buff（除漂浮外），buff持续1分钟
- 副作用：一次性物品，使用后消耗
- 合成方式：2个普通的符箓 + 1个元气 + 4个金锭

![](crafting%20table/txpb.png)

“一卷符箓聚万灵，诸般吉益随身行，片刻护持无缺憾，唯弃漂浮顺道心。”

### 武器类

道剑类拥有相对原版更强的面板和耐久，可使用开光符【启灵】获得附魔，部分剑具有特殊能力。

#### 桃木剑
- 核心功能：
  1. 可使用开光符【启灵】附魔亡灵杀手III
  2. 普攻一般生物：伤害略高于普通木剑
  3. 主手持有时：获得回血buff
- 合成方式：1个木剑 + 1个朱砂粉+1个炁 + 6个元气

![](crafting%20table/sword_t.png)

“桃木禀阳，能制阴邪，剑出则鬼魅潜藏，正气浩然。”

#### 宝剑
- 核心功能：
  1. 可使用开光符【启灵】附魔亡灵杀手III
  2. 普攻一般生物：伤害略高于「桃木剑」
  3. 主手持有时：抗性提升
- 合成方式：1个金剑 + 1个朱砂粉+1个炁 + 6个元气

![](crafting%20table/sword_b.png)

“宝剑锋从磨砺出，道心炼自精气神，一剑在手，可斩妖氛。”

#### 七星剑
- 核心功能：
  1. 可使用开光符【启灵】附魔亡灵杀手IV
  2. 普攻一般生物：伤害略高于「宝剑」
  3. 主手持有时：回血buff + 移速提升
- 合成方式：1个桃木剑 + 4张普通符箓 + 3个炁 + 1个元气

![](crafting%20table/sword_q.png)

“七星映剑，道法相随，执此剑者，上应天象，所向披靡。”

#### 令剑
- 核心功能：
  1. 可使用开光符【启灵】附魔亡灵杀手V
  2. 普攻一般生物：伤害略高于「七星剑」
  3. 主手持有时：抗性提升 + 移速提升
- 合成方式：1个宝剑 + 4张普通符箓 + 3个炁 + 1个元气

![](crafting%20table/sword_l.png)

“令剑承天令，威仪镇四方，挥剑斩虚妄，道力护其身。”

#### 拷鬼棒
- 核心功能：
  1. 可使用开光符【启灵】附魔亡灵杀手V和击退
  2. 主手持有时：获得抗性提升buff + 持续回血buff
  3. 主动技能（同时按住跳跃+攻击）：向前瞬移一段距离（可用于追击或规避）
- 合成方式：3个木棍 + 2个金锭 + 4个普通的符箓

![](crafting%20table/wond.png)

“拷鬼之棒承道威，专诛阴邪破魑魅，持之自有抗性护，瞬移追逃任我行。”

#### 奇怪的木剑【邪】
- 核心功能：
  1. 普攻伤害：略高于普通木剑
  2. 主手持有时：移速提升 + 跳跃提升
- 副作用：「合理的邪修」（无实际负面效果）
- 合成方式：1个木剑 + 1个噬魂术（经书） + 7个元气

![](crafting%20table/sword_e.png)

“邪道之剑，虽有小利，心若偏邪，终难成大道。”

#### 奇怪的木剑【异】
- 核心功能：主手持有时，移速提升 + 跳跃提升
- 副作用：玩家尝试攻击时，触发雷劈致残
- 合成方式：1个噬魂术（经书） + 1个桃木剑 + 7个元气

![](crafting%20table/sword_w.png)

“异术逆天，剑虽诡速，逞凶必遭天谴，悔之晚矣。”

#### 灵剑
- 核心功能：
  1. 综合能力：强于「令剑」
  2. 主手持有时：抗性提升 + 移速提升 + 回血buff，伴有持续饥饿效果
  3. 攻击会消耗自身精力，获得凋零效果
- 合成方式： 4个魂魄 + 4个炁 + 1个奇怪的木剑【邪】

![](crafting%20table/sword_sp.png)

“灵剑聚神魂，威力无穷，然耗己精元，修行当知取舍。”

#### 万魂幡
- 核心功能：
  1. 主手持有时：对30×30×30范围内生物持续震慑 + 群伤，伴有持续饥饿效果
  2. 使用时：将幡中残缺的灵魂释放，凝聚成4只旱魃实体协同战斗，消耗四个魂魄的充能值
- 副作用：
  1. 主手持有时会获得饥饿效果
  2. 主手持有时消耗自身精力，获得凋零效果
  3. 需使用魂魄充能，如强制使用则扣除自身生命
- 合成方式：1个黑色旗帜 + 1个灵剑 + 2个炁 + 3个魂魄 + 2个普通的符箓

![](crafting%20table/soul_banner.png)

“万魂聚幡，威力骇人，有伤天和，非正道所取，慎用之。”

“道友,你这人皇幡怎么库库冒黑烟?”

#### 人皇幡
- 核心功能：
   1. 将部分“战友”的灵魂释放，凝聚成玄门道士在使用者四周，辅助使用者战斗，持续时间30秒。
   2. 使用人皇幡右键玄门道士重置计时时间。
- 副作用：无特殊负面效果
- 合成方式：1个黄色旗帜 + 1个桃木剑 + 2个炁 + 3个朱砂粉 + 2个普通的符箓

![](crafting%20table/hy_banner.png)

“人皇者，亲友庇护也，攻防一体，幡承其气，可化万象。”

#### 百花花蓝
- 唯一获取方式：通过击败凋零有几率随机掉落百花花篮/清净莲花其中之一
- 核心功能：
  1. 使用时：会在使用者周围生成花丛
  2. 对周围生物产生震慑或减速效果
- 副作用：无特殊负面效果

![](Taoism_R/textures/items/fbusket.png)

“百花花篮集草木之灵，藏春和之韵，展之则芳华遍地，引生机而佑四方。”

#### 清净莲花
- 唯一获取方式：通过击败凋零有几率随机掉落百花花篮/清净莲花其中之一
- 核心功能：
  1. 使用时：会对以使用者为中心的半径25格内的生物产生震慑效果
  2. 使用时：对范围内的生物持续扣血，自身回复血量
- 副作用：无特殊负面效果

![](Taoism_R/textures/items/lotus.png)

“清净莲花出淤泥而不染，纳清宁之息，持之可涤心尘，离烦扰而证道心。”

#### 乾坤圈
- 核心功能：
  1. 使用后可在一定时间内获得乾坤圈的加持，在此期间不会受到任何伤害，同时暂时失去攻击外物能力
  2. 乾坤圈加持期间，玩家连续点按攻击会持续积攒能量，并在收起乾坤圈后统一释放
- 3. 使用后进入冷却，可借助「息」达到更高的伤害等级
- 合成方式：8个铜锭 + 1个补火符

![](crafting%20table/shield_q.png)

"乾坤圈转，阴阳自守，蓄势待发，一击破苍穹。"

#### 铜钱剑
- 核心功能：下蹲攻击释放铜钱剑上前，自动锁定多个目标后逐一攻击，结束后自动收回
- 获取方式：可在流浪商人处花25绿宝石购买

![](Taoism_R/textures/items/sword_m.png)

"青蚨串作斩邪锋，出匣飞腾逐乱凶。击罢群妖还自返，一枚铜气凛霜风。"

#### 纯阳飞剑
- 唯一获取方式：通过在副手装备清阳之炁可凝聚纯阳之炁为剑
  1. 主动技能：同时按空格键 + 鼠标左键，向前瞬移一段距离，并获得缓降效果
  2. 主动技能：长按潜伏蓄力，蓄力完成后震慑包括其他玩家在内的实体，随后进入冷却
  3. 同时按shift + 鼠标右键收起佩剑

![](Taoism_R/textures/items/sword_qi.png)

“纯阳飞剑凝正阳之炁，剑出如雷，斩妖除邪，千里之外可取魑魅之首。”

#### 赤旱尸煌剑
- 核心功能：
  1. 主手持有时，强制改为天气为晴天，并且削除周身5×5×5的水方块。
  2. 主手持有时，将周身3×3的草方块变成泥土，每隔3秒在玩家脚底生成3×3的火，玩家自身免疫火焰伤害和岩浆伤害。
  3. 下蹲攻击：向前释放火焰，并且火焰可摧毁路径上的方块，并对路径上的生物造成伤害，高度两格，长度10格。
  4. 跳跃攻击：释放两个环绕粒子，粒子可吸附地怪，造成持续伤害。
- 副作用：拥有旱魃的力量也会受到旱魃尸丹的反噬，使用时会持续饥饿。
- 合成方式：1个奇怪的木剑（邪） + 2个尸丹 + 2个朱砂晶体

![](crafting%20table/sword_za.png)

“旱既大甚，涤涤山川。旱魃为虐，如惔如焚。我心惮暑，忧心如熏。”

#### 五雷镇旱七星剑
- 核心功能：
  1. 主手持有时，提高血量上限到30个心，并有回血效果，玩家免疫火焰伤害。
  2. 主手持有时，会将脚下5×5的岩浆变成黑曜石。
  3. 下蹲蓄力强制降雨，并且能给自己以及10×10×10范围内队友或者是友好生物回血。
- 合成方式：1个七星剑 + 2个净炼尸丹 + 2个元气

![](crafting%20table/sword_zb.png)

“雷车轣辘起层阴，雨脚垂垂一寸深。解得田畴千里旱，全归天地一言心。”

#### 承影剑
- 核心功能：
  1. 下蹲可释放剑分身和圆形光环，剑分身指向最近敌对生物，光环持续时间25秒。
  2. 存在剑分身时攻击可使光环内生物同步受击。
  3. 剑分身没有上限。
- 合成方式：1个令剑/七星剑 + 2个普通符箓 + 2个朱砂粉 + 2个炁 + 2个阴魄砂

![](crafting%20table/sword_d.png)

"承影分形逐寇踪，环涵罡影护寒锋。挥刃群邪齐应刃，千身万影任横纵。"

#### 阴阳追魄剑
- 核心功能：
  1. 攻击和积攒怨气，怨气分为3个等级，随怨气等级提高而获得伤害加成。
  2. 怨气等级大于2时下蹲释放冲击波攻击，消耗2级怨气值并造成少量伤害。
  3. 怨气大于3进入超载状态，超载状态下持续扣血，伤害加成提升至等级4
- 合成方式：1个灵剑 + 2个普通符箓 + 2个魂魄 + 2个炁 + 2个阴魄砂

![](crafting%20table/sword_c.png)

"阴阳追魄淬寒铓，怨积三阶势渐张。蹲震阴芒摧烈浪，溢怨焚躯刃逾刚。"

#### 天魔诛仙剑
- 核心功能：
  1. 魔教至凶飞剑，源自五台派的镇派邪剑，魔道第一杀剑，邪修毕业武器
  2. 可御剑飞行
  3. 蹲下时攻击召唤剑灵协同攻击
- 剑灵：
  1. 召唤时，自动标记范围20格内生物，剑灵将自动攻击被标记的生物，攻击可追加标记
  2. 不存在目标时剑灵将为玩家提供生命恢复效果
- 副作用：攻击时由于需要驱动剑灵耗费精力，获得凋零效果
- 合成方式：2个金刚琢 + 1个万魂幡 + 2个法钟 + 1个金雷镇煞符 + 2个炁 + 1个金块

![](crafting%20table/sword_u.png)

“天魔诛仙剑凝九天煞气，剑出则魔灭神惊，斩尽诸天魔障，道威浩荡无匹。”

#### 三五雌雄斩邪剑
- 核心功能：
  1. 道教第一法剑，源自太上老君的佩剑，为道教传承象征，正修毕业武器
  2. 持剑者获得无敌效果，可御剑飞行，攻击时秒杀一切目标
  3. 下蹲时释放八道闪电，可自动追踪附近的生物，有短暂冷却
- 副作用：容易把自己帅死（无实际负面效果）
- 合成方式：2个金刚琢 + 1个人皇幡 + 1个令剑 + 1个七星剑 + 1个三清镇元三台符 + 2个炁 + 1个清阳之炁

![](crafting%20table/sword_s.png)

“三五之数，合于天地，雌雄双剑，斩尽邪祟，大道昌明。”

### 法器类

#### 雷击木
- 核心功能：放置于物品栏第一格时，对10×10×10范围内生物造成减速
- 合成方式：1个雷击符 + 4颗橡木树苗

![](crafting%20table/lightning_wood.png)

“雷击之木，禀天雷之气，镇于一方，百兽皆惧，邪祟不侵。”

#### 铜镜
- 核心功能：使用后获得增益——移速提升1 + 跳跃提升1 + 最大血量提升100%
- 副作用：
  1. 一次性物品，使用后消耗
  2. 增益持续30秒「门外汉的把戏罢了」
- 合成方式：8个铜锭 + 1个朱砂粉

![](crafting%20table/mirror_a.png)

“铜镜照形，亦能鉴心，一点朱砂，引气入体，暂得神力。”

#### 铜镜【法】
- 核心功能：使用后获得增益——移速提升2 + 跳跃提升2 + 最大血量提升100%
- 副作用：
  1. 一次性物品，使用后消耗
  2. 增益持续1分钟「不建议使用」
- 合成方式：8个铜锭 + 1个普通的符箓

![](crafting%20table/mirror_b.png)

“法镜含道气，一照百邪消，增益虽佳，终是外物，修行在己。”

#### 凸八卦镜
- 核心功能：使用时，对15×15×15范围内生物持续震慑，亡灵生物持续扣血
- 副作用：「亮出来就是吓唬人的」，一次仅持续10秒
- 合成方式：8个铜镜【法】 + 1个桃木剑

![](crafting%20table/mirror_c.png)

“八卦定乾坤，凸镜镇凶煞，镜光所及，邪祟退散。”

#### 凹八卦镜
- 核心功能：使用时，使用者及附近队友获得——最大血量提升100% + 持续快速回血
- 副作用：
   趣味设定「见过一次的把戏就不灵了」，buff持续1分钟
- 合成方式：8个铜镜【法】 + 1个元气

![](crafting%20table/mirror_d.png)

“凹镜纳灵气，八卦聚生机，护己及人，道之仁心。”

#### 火丹
- 核心功能：主手持有火丹时可将其发射出去
- 合成方式：1个尸丹碎片 + 1个煤炭 + 1个黏土

![](Taoism_R/textures/items/fball.png)

"火凝丹丸掌中擎，脱手星飞烈焰生。一瞬燎原三百丈，妖邪遇尽化尘轻。"

#### 铁拐
- 核心功能：
  1. 提高使用者的移速，分为三档（副手手持/主手手持/主手手持攻击）。
- 合成方式：3个木棍 + 2个铁锭 + 2个雷击木 + 2个元气

![](crafting%20table/cane.png)

"铁拐拄地行如风，疾步轻身御气通。"

#### 法钟
- 核心功能：
  1. 主手持有时：抗性提升拉满
  2. 使用时：对半径25格的范围内生物持续震慑，同时敌对生物会受法音攻击
- 副作用：主手持有时，自身移速降低 + 饱食度降低「很可惜，你跑不快」
- 合成方式：2个炁 + 5个铜锭 + 1个净炼尸丹

![](crafting%20table/bell.png)

“法钟鸣，大道兴，钟声荡邪，抗性加身，宁心定志。”

#### 紫金葫芦
- 核心功能：
  1. 使用时：主动吸引20×20×20范围内所有物品
  2. 或破坏面前方块并吸收
  3. 可主动吸引炁
- 合成方式：5个铜锭 + 1个普通的符箓 + 1个元气

![](crafting%20table/gourd.png)

“葫芦藏天地，能纳万物灵，收放之间，合于自然。”

#### 玉笏
- 核心功能：向天庭请示后，可瞬移至亲友身旁
- 副作用：瞬移后扣除1点血（半颗心）「由于你主观离岗，上天会扣除代价」
- 合成方式：3个雷击木（木板） + 1个普通的符箓

![](crafting%20table/wul.png)

“笏板承天语，一念至亲朋，瞬移虽便，亦有代价，道在平衡。”

#### 拂尘
- 核心功能：
  1. 清除自身所有buff「扫除心尘，去掉自己身上所有buff」
  2. 可作为武器使用
- 合成方式：1个木棍 + 1个白色羊毛

![](crafting%20table/whisk.png)

“拂尘扫尘埃，亦能清心障，一拂之下，返璞归真。”

#### 金刚琢
- 核心功能：老君的另一件法宝，蓄力后释放金刚琢逐一攻击并缴械附近生物，使其失去攻击能力
- 合成方式：4个铁锭 + 4个雷击木 + 1个普通的符箓

![](crafting%20table/ring.png)

“金刚琢，镇万物，缴械制敌，物极必反，此乃道之理。”

- 这件兵器，乃锟钢抟炼的，被我将还丹点成，养就一身灵气，善能变化，水火不侵，又能套诸物；一名‘金钢琢’，又名“金钢套”。
- 当年过函关，化胡为佛，甚是亏他。早晚最可防身。等我丢下去打他一下。
- 收过金箍棒的法宝，实则正史没记载过这样的法宝，也没有金箍棒。

#### 净瓶
- 核心功能：
  1. 主手持有时，为附近生物提供持续回血效果
  2. 副手持有时，「魂魄」不再提供充能效果，被净化成「元气」
- 副作用：无负面副作用，可重复使用（非一次性道具）
- 合成方式：3个黏土块 + 1个普通的符箓 + 1个下界之星

“净瓶含甘露，滴之生生机，普济众生，道之慈悲。”

![](crafting%20table/bottle.png)

#### 令牌【通天术】
- 核心功能：抬头向上亮出令牌，请示神明，可隔墙向上传送
- 使用方式：点击进入预览视角，下蹲使用传送，跳起取消传送
- 合成方式：2个雷击木+1个末影珍珠+2个普通符箓

![](crafting%20table/tup.png)

“令牌承天命，举之可通天，隔墙传送，应乎神道。”

#### 浑仪
- 核心功能：使用可观测星空，明确月相，以作修行参考
- 使用方式：左键单击使用
- 合成方式：3个木原木+2个木棍+1个铜锭+3个木台阶

![](crafting%20table/hy.png)

“浑仪窥月辨盈亏，修经必循月相规，逆道妄为遭天谴，天雷一击丧魂归。”

#### 昆仑镜
- 核心功能：使用后使用者与周围最近的实体互换位置
- 使用方式：左键单击使用
- 合成方式：9个普通符箓+1个铜块

![](crafting%20table/klj.png)

“昆仑宝镜映乾坤，一念互换定方位，咫尺之间移形影，道法玄机藏镜中。”

#### 枯魂旱火葫芦
- 核心功能：置于副手后，主手空手可凝练出火丹
- 使用方式：置于副手
- 合成方式：1个紫金葫芦 + 4个尸丹碎片

![](crafting%20table/firegourd.png)

“枯魂锁入葫中藏，旱火凝丹发烈光。一点阴焰焚万物，赤地千里草不芳。”

#### 流辉葫芦
- 核心功能：置于副手后，可自动为冷却取消条充能，每两秒充能一次
- 使用方式：置于副手
- 合成方式：1个紫金葫芦 + 4个净炼尸丹

![](crafting%20table/spegourd.png)

“流辉敛炁纳元光，净炼残阴入此囊。袖底自充仙府力，不劳执手运丹方。”

- 原来有个特别善用法宝的道友，苦于元气难以聚集。后来他腰间多了个水蓝的葫芦，身边的法音也是不带停的。

#### （泰山）石敢当
- 核心功能：置于地表上，每5秒对附近15×15范围内的所有敌对生物产生伤害与减速
- 合成方式：8个深板岩 + 1个画符毛笔

![](crafting%20table/sguard.png)

“泰山片石号敢当，坐镇方隅慑不祥。魑魅逢之皆敛迹，郊原十里尽安昌。”

#### 八卦罗盘
- 核心功能：
  1. 使用者使用后，在半径25格的球形范围内，所有生物获得减速效果
  2. 给自身和队友施加回血效果，给其他所有生物施加持续扣血的debuff
  3. 离使用者越近，效果越强，但使用者不能移动
- 使用方式：左键单击使用
- 合成方式：4个普通符箓+4个钻石+1个指南针

![](crafting%20table/bglpa.png)

“八卦罗盘定八方，五十丈内气场强，静立方能承道力，一动则失此中章。”

#### 浊阴之炁
- 核心功能：使用后原地进行“暴炁”，对四周产生爆炸伤害，本体增加抗性
- 使用方式：右键单击使用
- 合成方式：通过吸收23个“炁”在内丹中自动炼化而成，可与浊阴之炁互相转化

![](Taoism_R/textures/items/qi_c.png)

“浊阴之炁沉于地，含纯阴之质，滋养万物，驭之可通幽冥而化万象。”

#### 清阳之炁
- 核心功能：置于副手后，主手空手时可召唤「纯阳飞剑」
- 使用方式：置于副手手持，手持状态不再显示，放回主手恢复
- 合成方式：通过吸收23个“炁”在内丹中自动炼化而成，可与清阳之炁互相转化

![](Taoism_R/textures/items/qi_sword.png)

“清阳之炁升于天，禀纯阳之精，涤荡阴秽，修行得之则神清气朗。”

#### 天蓬尺
- 核心功能：置于主/副手后，自动感应附近的危险生物
- 使用方式：置于主/副手后，感应四周，如有危险生物则震屏警告并发射跟踪粒子
- 获取方式：游商处购买

![](Taoism_R/textures/items/ruler.png)

“天蓬法尺秉威灵，暗察妖氛动即鸣。飞珠逐影无虚发，不遣凶邪近身形。”

#### 上清景震剑
- 核心功能：召唤可飞行的剑
- 使用方式：
  - 右键/交互：召唤飞剑，可骑乘飞行
  - 左键/攻击：将最近的飞剑移至自己
  - 下蹲左键/攻击：收回飞剑
- 获取方式：完成古书支线后从神秘道士处获得

![](Taoism_R/textures/items/sword_f.png)

“上清敕赐景震锋，驾御飞虹碧落中。招即还身收即隐，纵横千里驭罡风。”

#### 摄魂铃
- 核心功能：控制封印后的玩家尸体
- 使用方式：
  - 玩家的尸体使用普通的符箓封印变为封印态
  - 封印态下使用摄魂铃可吸引封印的尸体
- 合成方式：2个铜锭 + 1个元气 + 1个木棍

![](crafting%20table/bell_a.png)

"摄魂铃振驭阴行，符定形骸始听令。行止皆随铃畔引，解印俄顷复归冥。"

#### 法印
- 核心功能：简化请神仪式直接请神，获得神力——抗性提升和环绕助战粒子
- 使用方式：
  - 攻击启用效果，获取高抗性，并释放四个粒子
  - 粒子会持续吸引敌怪并持续造高额伤害
- 合成方式：4个铜块 + 2个炁 + 2个精炼尸丹 + 1个绿宝石

![](crafting%20table/seal.png)

"法印催灵布御罡，四炁分形助战铓。诸般法滞都消尽，一印机行不可返。"

#### 摄妖印
- 核心功能：改变敌对生物的索敌
- 使用方式：
  - 右键模组僵尸使其改为攻击敌对生物
- 合成方式：4个铜块 + 2个炁 + 2个尸丹碎片 + 1个噬魂术（经书）

![](crafting%20table/seal_e.png)

"摄妖宝印敕阴锋，转易尸心向邪踪。不向修途凌行客，专随法令剿群凶。"

### 其他

#### 家具

游商处购买，可放置和使用。

已有家具：
- 画符桌：可用于符合成
- 立柜：储存物品
- 茶桌
- 茶凳
- 椅子
- 自行车：可骑乘

#### 复活图腾
- 核心功能：
  - 召唤一个队友，使用朱砂晶体驯服（？）
  - 可以为他装备原版武器
  - 可以提供额外储存空间
  - 可能会告诉你一些信息（？）
- 合成方式：3个朱砂晶体 + 1个绿宝石 + 1个不死图腾 + 4个炁

![](crafting%20table/partner.png)

“有朋相伴好过孤军奋战。”

- 杀人成风之后的那个时候，有的权贵想让自己在乱世后重新复活，可不知怎的，最后出现的总是另一个素不相识的人。
- 据说老道长死后，手里攥着的是个奇怪的木头玩意，村里人便将其挂在了观里。不知为何，多年过去了，那座观还是崭新如初。

#### 主题唱片
- 在末地城中获得
- 可播放主题曲《轮回》，点击右键(使用)播放，左键(攻击)停止

![](Taoism_R/textures/items/disk_taoism.png)

"一曲轮回绕梁间，道音袅袅入心田。"

## 26年春节更新内容

![](Taoism_R/textures/items/ny/dumpling_a.png)
![](Taoism_R/textures/items/ny/pigskin_jelly.png)
![](Taoism_R/textures/items/ny/scrambled_chicken.png)
![](Taoism_R/textures/items/ny/braised_pork.png)
![](Taoism_R/textures/items/ny/braised_fish.png)
![](Taoism_R/textures/items/ny/noodles.png)
![](Taoism_R/textures/items/ny/steamed_bun.png)
![](Taoism_R/textures/items/ny/rice_cake.png)

可从村民处(屠夫，农民)购买食材，制作年夜饭

部分饭菜可提供特殊效果

食物效果：
- 饺子使用牛肉，羊肉，猪肉时分别提供力量，抗性提升，生命恢复，使用葱，白菜分别指定作用时间3分钟，5分钟
- 猪皮冻，红烧肉提供生命恢复，红烧肉的效果优先
- 炒鸡可解除虚弱，挖掘疲劳，凋零
- 红烧鱼可解除缓慢，中毒，失明
- 其余食材仅提供饱食度回复，调味品不可食用

替换部分画替换为“福”字

## 

### 「道生一，一生二，二生三，三生万物。」——《道德经》第四十二章
### 「上善若水，水善利万物而不争，处众人之所恶，故几于道。」——《道德经》第八章
### 「修之于身，其德乃真；修之于家，其德乃余；修之于乡，其德乃长。」——《道德经》第五十四章
### 「天地与我并生，而万物与我为一。」——《庄子・齐物论》
## 在方块搭建的天地间，这些符箓、剑器与丹炉旁的微光，
## 恰是对“道在蝼蚁，在稊稗，在瓦甓，在屎溺”的游戏化注解——
## 道教文化中“天地与我并生，而万物与我为一”的哲思，
## 化作了桃木剑劈开亡灵时的微光，
## 化作了缩地符记录坐标时的闪烁，
## 也化作了正邪路径选择时的微妙平衡。
## 当你手持七星剑在月夜下斩除魑魅，
## 或是以万魂幡引雷时感受力量的代价，
## 实则是在与千年文化中“修之于身，其德乃真”的修行观对话。
## 那些道馆里的引导书、经书里的真与假、符箓上的朱砂印记，
## 既是玩法的密钥，
## 也是对“道生一，一生二，二生三，三生万物”的趣味演绎：
## 从基础的朱砂晶体到终极的三五雌雄斩邪剑；
## 从正道的步步为营到邪道的快意恩仇，
## 恰如阴阳相生相克，
## 在虚拟的修行路上映照着“反者道之动”的朴素智慧。
## 或许游戏终有边界，
## 但对“道”的探索永无止境。
## 愿你在方块世界的符箓光影里，
## 既能体验仙术的奇幻，
## 也能读懂那份“上善若水，利万物而不争”的从容——
## 毕竟，最好的修行，从来都在脚下的每一步探索里……
## ——开发者：韩言悦欣
## ——开发者：zz纸张神探st

![](model/dev_1k.png)